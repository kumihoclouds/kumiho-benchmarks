...
- supersedes: belief updates where a NEW fact REPLACES a prior one — the speaker's job, home, plan, or status CHANGED. Emit one when the summary itself signals a change ("no longer", "used to", "now", "instead of", "changed to", "moved from ... to") AND states the prior fact's content. Each: {{"statement": str, "replaces": str}} where "statement" is the CURRENT fact and "replaces" is the PRIOR fact it overrides, phrased as a standalone statement.
...
Extract at most 10 of each. Only include a relation when both endpoints are entities you listed. For supersedes/contradicts, BOTH the "statement" and its "replaces"/"conflicts_with" target MUST ALSO appear as separate entries in the facts list — a belief change whose two statements are not both in facts will be dropped. Emit a supersedes/contradicts entry ONLY when both the new fact AND the prior fact's content are recoverable from this summary's own text, stated or directly paraphrasable (e.g. "left her job at the bakery to join Google" gives both facts). When the summary signals a change but does not state what the prior was (e.g. "now works at Google" with no prior employer mentioned), put the NEW fact in facts as usual and SKIP the belief entry. NEVER invent the prior fact's specifics. If a category has nothing, use an empty list.
